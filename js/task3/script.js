...
// Initial Array
const goods = [{ ratingRevievs: "264 отзыва", price: { oldUan: "4 333 грн", newUan: "3 799 грн" }, name: "Motorola MOTO G4 (XT1622) Black" },
...
//sort by rating
const goodsByRating = goods.slice().sort((a, b) => parseInt(b.ratingRevievs) - parseInt(a.ratingRevievs));

//sort by price 
// I understood that i could check by typeof only after writing this part
// typeof(a.price) === "object" && typeof(b.price) === "string"  and ...
const goodsByPrice = goods.slice().sort((a, b) => {
    if (Object.keys(a.price).length === 2 && Object.keys(b.price).length > 2) {
        return parseInt(a.price.newUan.replace(" ", "")) - parseInt(b.price.replace(" ", ""));
    }
    else if (Object.keys(a.price).length > 2 && Object.keys(b.price).length === 2) {
        return parseInt(a.price.replace(" ", "")) - parseInt(b.price.newUan.replace(" ", ""));
    }
    else if (Object.keys(a.price).length === 2 && Object.keys(b.price).length === 2) {
        return parseInt(a.price.newUan.replace(" ", "")) - parseInt(b.price.newUan.replace(" ", ""));
    }
    else {
